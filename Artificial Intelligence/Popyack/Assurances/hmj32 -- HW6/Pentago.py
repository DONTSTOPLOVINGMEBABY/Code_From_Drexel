...
#--------------------------------------------------------------------------------
	...
	#---------------------------------------------------------------------------
		...
		'''
		for i in moves_from_minimax:
			print(self.monteCarlo(board, i[0]))
		'''

		return move


	def monteCarlo(self, board, move):
		
		opponent_token = "w" if self.token == "b" else "b"		
		newBoard = copy.deepcopy(board)
		newBoard = board.applyMove(move, self.token)
		opponent = Player("TempPlayer", playerType="computer", token=opponent_token)
		current_token = copy.deepcopy(self.token)

		win0 = self.win(newBoard)
		win1 = opponent.win(newBoard)
		numEmpty = newBoard.emptyCells
		...
		while not gameOver:
			...
			randomInt = random.randint(0, len(moves)-1)
			random_move = moves[randomInt]
			newBoard = newBoard.applyMove(random_move, current_token)

			if current_token == self.token:
				sum_heuristic += self.hmj32_h(board)
...
#--------------------------------------------------------------------------------
	...
	while( not gameOver ):
		...
		print(player[currentPlayer].hmj32_h(newBoard))							### Prints heuristic Value for current player
