...
#--------------------------------------------------------------------------------
	...
	#---------------------------------------------------------------------------
		...
		for each_my_move in moveList:
			...
			for each_their_move in nextState.getMoves():
				...
				if (depth == maxDepth):
					once = self.hmj32_h(newState)
					return each_my_move, once
					
				else:
					min = self.hmj32_h(newState)
					self.miniMax(newState, min, depth + 1, maxDepth)
			...
			if min >= max:
				...
				move = each_my_move
				tuppy = (move, max)
				moves_from_minimax.append(tuppy)
	...
	#---------------------------------------------------------------------------
		...
		global moves_from_minimax 	## list of moves returned by minimax as best moves... some moves will have to be pruned in order to get list of best
		moves_from_minimax = [] 
		list_of_moves = [] 
		best_n_moves = 5			## how many moves monte_carlo will play (if there are this many moves available)

		
		opponent = "w" if self.token=="b" else "b"
		move, max = self.miniMax(board, self.INFINITY, 0, 1)


		#print(move, max)

		if (len(moves_from_minimax)>0):

			max = moves_from_minimax[len(moves_from_minimax)-1][1]	## grabs the last element from moves_from_minimax which should be the max moves returned by minimax
		elif (len(moves_from_minimax) == 1):
			max = moves_from_minimax[0]
		

		ctr = 0 	
		for move in moves_from_minimax:							## takes n of the best moves
			if move[1] == max and ctr < best_n_moves:
	...
	def MonteCarlo(self, list_of_moves, board):
...
#--------------------------------------------------------------------------------
	...
	print(player[currentPlayer].playerMove(pb))


	while( not gameOver ):
		move = player[currentPlayer].playerMove(pb)[0]
		if move == "exit":
			break
